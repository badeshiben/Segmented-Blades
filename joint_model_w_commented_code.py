...
"""WISDEM segmented blade joint pseudocode
...
# safety factors
nprf = 2  # bolt static safety factor (proof) [-]. If this is too high, it seriously constrains the design.
ny   = 2    # insert static safety factor (yield) [-]
nf   = 2    # fatigue safety factor [-]
n0   = 1.5    # separation safety factor [-]. If this is too high, it seriously constrains the design
...
Mflap_static    = 1.64e6 * nDLC61  # Nm 50 for e50?
Mflap_fatigue   = Mflap_static * 0.3e-3  # Nm
Fflap_static    = 1.5e5 * nDLC61  # N
Fflap_fatigue   = Fflap_static * 0.3e-3  # N
Medge_static    = 3.1e5 * nDLC61  # Nm
Medge_fatigue   = Medge_static * 0.3e-3                            # Nm
...
Fi70p   = 0.7 * Sp_bolt * At  # N, per bolt
Fi      = Fi70p
...
def fxn(x):
    return x ** 2 * ((Fi / A_insert) ** 2 - (Sy_insert / ny) ** 2) + x * 2 * Fi * (
                (1 - C) * Fax_flap_static) / A_insert ** 2 + 3 * (Fsh_flap_static / A_insert) ** 2
...
while n_bolt != n_bolt_prev:  # find a way to break out of a Fi-change-loop
    ...
    # bolts
    n_bolt_flap_static = C * Fax_flap_static / (Sp_bolt * At / nprf - Fi)
    n_insert_ax = (1 - C) * Fax_flap_static / (Sy_insert * A_insert / ny - Fi)
    n_insert_sh = ns * Fsh_flap_static * np.sqrt(2) / (Sy_insert * A_insert)  # Ss=sqrt(2)Sy
    # inserts. Could consider torsion here. This equation is giving incorrect results. IDK why. Keep an eye on it, but
    # inserts shouldn't be the weakest link here.
    # a = ((Fi / A_insert) ** 2 - (Sy_insert / ny) ** 2)
    # b = 2 * Fi * ((1 - C) * Fax_flap_static) / A_insert ** 2
    # c = 3 * (Fsh_flap_static / A_insert) ** 2
    # x1 = abs((-b + np.sqrt(b ** 2 - 4 * a * c)) / (2 * a))
    # x2 = abs((-b - np.sqrt(b ** 2 - 4 * a * c)) / (2 * a))
    # n_insert_flap_static = max([x1, x2])
    ...
    # tau_insert = Fsh_flap_static/A_insert
    # sig_ax_insert = ((1-C)*Fax_flap_static+Fi)/A_insert
    # n_insert_flap_static = ny/Sy_insert*np.sqrt(sig_ax_insert**2 + 3*tau_insert**2)
    # --
    # root = fsolve(fxn, 10)
    # n_insert_flap_static = root
    ...
    # calc # bolts/inserts needed to satisfy adhesive shear (von-mises? Only has compression and shear) requirements
    # REMOVE FOR FIRST PASS
    # ni_adhesive = Fax_flap_static/(S_adhesive*A_adhesive)
    ...
    n_bolt = np.ceil(np.max([n_bolt_flap_fatigue, n_bolt_flap_static, n_insert_ax, n_insert_sh, n_insert_flap_static, n_insert_flap_fatigue, n_bolt_pullout, n_bolt_min]))
    ...
    if n_bolt_flap_static < 0 or n_bolt_flap_fatigue < 0 or n_insert_sh < 0 or n_insert_ax < 0 or n_bolt_pullout < 0:
        print('Warning: negative bolt number found. This implies an imbalance in input loads. Please check inputs')
    ...
    Fi_sep = n0 * Fax_flap_static * (1 - C) / n_bolt
    ...
    print('n_insert_ax', n_insert_ax)
    print('n_insert_sh', n_insert_sh)
...
# below is a von mises calculation, unused for now because carbon fiber is so isotropic, and will only fail in shear
# Tau_pullout = Fax_flap_static/(A_adhesive*n_bolt)
# t1 = 2*Fsh_flap_static/(L_insert*n_bolt)*((Ss_sparcap/ns)**2-Tau_pullout**2)**(-1/2)
...
V_hole = w_hole * h_hole * np.pi * w_hole ** 2 / 4
V_hole_tot = V_hole * n_bolt
V_insert_tot = V_insert * n_bolt
V_cutout = V_insert_tot + V_hole_tot
